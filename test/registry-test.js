...
describe('BlockHeaderRegistry', async () => {
	...
	describe('Adding signed blocks', () => {
		it('Should properly rlp encode', async() => {
			const signer = signers[0]
			const header = {
			...
			const rlpEncoded = ethers.utils.RLP.encode(Object.values(header))
			const _rlpEncoded = await blockHeaderRegistry.connect(signer)._rlpEncode(Object.values(header).map(v => v == '0x' ? 0 : v))
			expect(rlpEncoded).to.equal(_rlpEncoded);
		})
		it.skip('Should add a standard EVM signed block', async () => {
			const signer = signers[0]
